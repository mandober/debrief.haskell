...
Since GHC 8.0.1, you can bind values and functions to names without
`let` statement:
...
::: {.ghc-flag shortdesc="Show the names of modules that GHCi loaded after a
...
Interactive evaluation at the prompt {#interactive-evaluation}
------------------------------------
...
### What\'s really in scope at the prompt? {#ghci-scope}
...
The interactive classes (only relevant when
...
The `:list`{.interpreted-text role="ghci-cmd"} command lists the source
...
The flag `-fprint-evld-with-show`{.interpreted-text role="ghc-flag"}
...
As we demonstrated earlier (`breakpoints`{.interpreted-text
...
The header line is followed by ⟨l⟩ lines each containing one completion
...
to the rules described under `ExtendedDefaultRules`{.interpreted-text
...
GHCi as described in `ghci-modules-filenames`{.interpreted-text
...
`:type-at`{.interpreted-text role="ghci-cmd"} falls back to a general
`:type`{.interpreted-text role="ghci-cmd"} like lookup.

The `:type-at`{.interpreted-text role="ghci-cmd"} command requires
`:set +c`{.interpreted-text role="ghci-cmd"} to be set.
:::

::: {.ghci-cmd}
:undef; ⟨name⟩

Undefines the user-defined command ⟨name⟩ (see `:def`{.interpreted-text
role="ghci-cmd"} above).
:::
...
Unsets certain options. See `ghci-set`{.interpreted-text role="ref"} for
...
The `:uses`{.interpreted-text role="ghci-cmd"} command requires
`:set +c`{.interpreted-text role="ghci-cmd"} to be set.
:::
...
`:def`{.interpreted-text role="ghci-cmd"}.
:::
...
The `:set` and `:seti` commands {#ghci-set}
-------------------------------

::: {.index}
single: :set; command in GHCi single: :seti
:::

The `:set`{.interpreted-text role="ghci-cmd"} command sets two types of
options: GHCi options, which begin with \"`+`\", and \"command-line\"
...
At the moment, the `:set`{.interpreted-text role="ghci-cmd"} command
doesn\'t support any kind of quoting in its arguments: quotes will not
be removed and cannot be used to group words together. For example,
`:set -DFOO='BAR BAZ'` will not do what you expect.
:::
...
GHCi options may be set using `:set`{.interpreted-text role="ghci-cmd"}
and unset using `:unset`{.interpreted-text role="ghci-cmd"}.
...
commands `:all-types`{.interpreted-text role="ghci-cmd"},
`:loc-at`{.interpreted-text role="ghci-cmd"},
`:type-at`{.interpreted-text role="ghci-cmd"}, and
`:uses`{.interpreted-text role="ghci-cmd"} require `+c` to be active.
:::
...
`ghci-multiline`{.interpreted-text role="ref"}).
:::
...
`:set`{.interpreted-text role="ghci-cmd"}. For example, to turn on
`-Wmissing-signatures`{.interpreted-text role="ghc-flag"}, you would
say:
...
in `flag-reference`{.interpreted-text role="ref"}), may be set using
`:set`{.interpreted-text role="ghci-cmd"}. To unset an option, you can
...
`flag-reference`{.interpreted-text role="ref"} lists the reverse for
each option where applicable.

Certain static options (`-package ⟨pkg⟩`{.interpreted-text
role="ghc-flag"}, `-I⟨dir⟩`{.interpreted-text role="ghc-flag"},
`-i⟨dir⟩[:⟨dir⟩]*`{.interpreted-text role="ghc-flag"}, and
`-l ⟨lib⟩`{.interpreted-text role="ghc-flag"} in particular) will also
...
The `:set`{.interpreted-text role="ghci-cmd"} command modifies both, but
there is also a `:seti`{.interpreted-text role="ghci-cmd"} command (for
...
It would be undesirable if `MonoLocalBinds`{.interpreted-text
...
practice to use `:seti`{.interpreted-text role="ghci-cmd"} rather than
`:set`{.interpreted-text role="ghci-cmd"}, unless you really do want
them to apply to all modules you load in GHCi.

The two sets of options can be inspected using the
`:set`{.interpreted-text role="ghci-cmd"} and `:seti`{.interpreted-text
role="ghci-cmd"} commands respectively, with no arguments. For example,
...
`ghci-dot-files`{.interpreted-text role="ref"}. Then the interactive
...
-   The option `-XExtendedDefaultRules` is enabled, in order to apply
    ...
    `extended-default-rules`{.interpreted-text role="ref"}).
-   The Monomorphism Restriction is disabled (see
    `monomorphism`{.interpreted-text role="ref"}).

The `.ghci` and `.haskeline` files {#ghci-dot-files}
----------------------------------
...
When it starts, unless the `-ignore-dot-ghci`{.interpreted-text
role="ghc-flag"} flag is given, GHCi reads and executes commands from
the following files, in this order, if they exist:

1.  `{ghcappdata}/ghci.conf`{.interpreted-text role="file"}, where
    ⟨ghcappdata⟩ depends on your system, but is usually something like
    `$HOME/.ghc`{.interpreted-text role="file"} on Unix or
    `C:/Documents and Settings/user/Application
    Data/ghc`{.interpreted-text role="file"} on Windows.
2.  `$HOME/.ghci`{.interpreted-text role="file"}
3.  `./.ghci`{.interpreted-text role="file"}

The `ghci.conf`{.interpreted-text role="file"} file is most useful for
...
use `:seti`{.interpreted-text role="ghci-cmd"} rather than
`:set`{.interpreted-text role="ghci-cmd"} (see
`ghci-interactive-options`{.interpreted-text role="ref"}).
:::

Placing a `.ghci`{.interpreted-text role="file"} file in a directory
...
and C, you might put the following lines in `.ghci`{.interpreted-text
role="file"}:
...
(Note that strictly speaking the `-i`{.interpreted-text role="ghc-flag"}
flag is a static one, but in fact it works to set it using
`:set`{.interpreted-text role="ghci-cmd"} like this. The changes won\'t
take effect until the next `:load`{.interpreted-text role="ghci-cmd"},
though.)
...
Sourcing untrusted `./.ghci`{.interpreted-text role="file"} files is a
security risk. They can contain arbitrary commands that will be executed
as the user. Use `:set local-config`{.interpreted-text role="ghci-cmd"}
to inhibit the processing of `./.ghci`{.interpreted-text role="file"}
files.
:::
...
Additionally, any files specified with `-ghci-script`{.interpreted-text
role="ghc-flag"} flags will be read after the standard files, allowing
...
Don\'t read either `./.ghci`{.interpreted-text role="file"} or the other
...
repeatedly for multiple inputs. `-ignore-dot-ghci`{.interpreted-text
role="ghc-flag"} does not apply to these files.
:::
...
`:: ⟨builtin-command⟩`{.interpreted-text role="ghci-cmd"}, like
`::type 3`.
...
`-fobject-code`{.interpreted-text role="ghc-flag"} flag either on the
command line or with `:set`{.interpreted-text role="ghci-cmd"} (the
option `-fbyte-code`{.interpreted-text role="ghc-flag"} restores
...
`:reload`{.interpreted-text role="ghci-cmd"} command typically runs much
faster than restarting GHC with `--make`{.interpreted-text
role="ghc-flag"} from the command-line, because all the interface files
...
the flag `-fexternal-interpreter`{.interpreted-text role="ghc-flag"} is
...
profiling mode if `-prof`{.interpreted-text role="ghc-flag"} is in
effect, and in dynamically-linked mode if `-dynamic`{.interpreted-text
role="ghc-flag"} is in effect.
...
`-fexternal-interpreter`{.interpreted-text role="ghc-flag"}.

See also the `-pgmi ⟨cmd⟩`{.interpreted-text role="ghc-flag"}
(`replacing-phases`{.interpreted-text role="ref"}) and
`-opti ⟨option⟩`{.interpreted-text role="ghc-flag"}
(`forcing-options-through`{.interpreted-text role="ref"}) flags.
:::
...
-   We can use the profiler to collect stack traces when using GHCi (see
    `ghci-stack-traces`{.interpreted-text role="ref"}).
-   When compiling Template Haskell code with `-prof`{.interpreted-text
    role="ghc-flag"} we don\'t need to compile the modules without
    `-prof`{.interpreted-text role="ghc-flag"} first (see
    `th-profiling`{.interpreted-text role="ref"}) because we can run the
...
When using the flag `-fexternal-interpreter`{.interpreted-text
role="ghc-flag"} GHC will spawn and communicate with the separate
...
Providing `-pgmi ⟨/path/to/iserv-proxy⟩ <-pgmi ⟨cmd⟩>`{.interpreted-text
role="ghc-flag"} and
`-opti ⟨slave-ip⟩ -opti ⟨slave-port⟩ [-opti -v] <-opti ⟨option⟩>`{.interpreted-text
role="ghc-flag"} in addition to
`-fexternal-interpreter`{.interpreted-text role="ghc-flag"} will then
...
`-O`{.interpreted-text role="ghc-flag"} doesn\'t work with GHCi!
...
> `UnboxedTuples`{.interpreted-text role="extension"} or
> `UnboxedSums`{.interpreted-text role="extension"} language extensions.
> It is not always the case that code which enables
> `UnboxedTuples`{.interpreted-text role="extension"} or
> `UnboxedSums`{.interpreted-text role="extension"} requires
> `-fobject-code`{.interpreted-text role="ghc-flag"}, so if you *really*
> want to compile `UnboxedTuples`{.interpreted-text
> role="extension"}/`UnboxedSums`{.interpreted-text
> role="extension"}-using code to bytecode, you can do so explicitly by
> enabling the `-fbyte-code`{.interpreted-text role="ghc-flag"} flag. If
...
>
> Incidentally, the previous point, that `-O`{.interpreted-text
> role="ghc-flag"} is incompatible with GHCi, is because the bytecode
...
:   This should work, as long as your GHCi was built with the
    `-threaded`{.interpreted-text role="ghc-flag"} switch, which is the
...
:   This is the defined behaviour of `getContents`: it puts the stdin
    ...
    `:load`{.interpreted-text role="ghci-cmd"} or
    `:reload`{.interpreted-text role="ghci-cmd"} command.
...
I can\'t use `Control-C`{.interpreted-text role="kbd"} to interrupt computations in GHCi on Windows.

:   See `ghci-windows`{.interpreted-text role="ref"}.
...
[^3]: Note that in GHCi, and `--make`{.interpreted-text role="ghc-flag"}
    mode, the `-i`{.interpreted-text role="ghc-flag"} option is used to
    specify the search path for *source* files, whereas in standard
    batch-compilation mode the `-i`{.interpreted-text role="ghc-flag"}
