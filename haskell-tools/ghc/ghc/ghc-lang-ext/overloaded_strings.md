...
::: {.extension shortdesc="Enable overloaded string literals."}
OverloadedStrings
...
Enable overloaded string literals (e.g. string literals desugared via
the `IsString` class).
:::
...
`OverloadedStrings`{.interpreted-text role="extension"}) a string
literal has type `(IsString a) => a`.
...
it explicitly (for example, to give an instance declaration for it), you
can import it from module `Data.String`.

Haskell\'s defaulting mechanism ([Haskell Report, Section
4.3.4](http://www.haskell.org/onlinereport/decls.html#sect4.3.4)) is
extended to cover string literals, when
`OverloadedStrings`{.interpreted-text role="extension"} is specified.
Specifically:
