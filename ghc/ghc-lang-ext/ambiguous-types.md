...
- `AllowAmbiguousTypes`
- since: 7.8.1
- desc: Allow the user to write ambiguous types, and the type inference engine to infer them.
...
The idea is there can be no legal calls to `f` because every call will give rise to an ambiguous constraint. Indeed, the *only* purpose of the ambiguity check is to report functions that cannot possibly be called.

We could soundly omit the ambiguity check on type signatures entirely, at the expense of delaying ambiguity errors to call sites. The language extension `AllowAmbiguousTypes` switches off the ambiguity check.
